    ...
    */
    Date.prototype.AddMonth = function (intMonth) {
        ...
        if (ModMonth > 0) {
            ...
            currYear = currYear - ((Math.abs(ModMonth) + 1) / 12);
        }
        if (ModMonth > 0 && ModMonth != 12) {
    ...
    Date.prototype.AddDay = function (intDay) {
        var currDate = this;
        return '/Date(' + currDate.setDate(currDate.getDate() + intDay) + ')/';
    }
    $.extend({
        ...
        ajaxStart: function (title) {
            if (title)
                this.progressStart(title)
            else
        ...
        progressStart: function (message) {
            $.messager.progress({ text: '<span style="color:#aaa;">' + message + '</span>', interval: 100 });
        },
        ...
        formatJsonDate: function (jsonDate, formatterString) {
            try {
                ...
                if (formatterString != undefined) {
                    return String.Format(formatterString, year, month, day, hours, minutes, seconds, milliseconds);
                } else {
                ...
                switch (elesConfig[i].Style) {
                    case 0:
                        ...
                        try {
                            ...
                            if ($('#orderform_' + formId + ' [name="' + elesConfig[i].IdentityString + '"]')[0].type == "textarea") {

                                if (elesConfig[i].OptionValues != '') {
                                    $('#orderform_' + formId + ' [name="' + elesConfig[i].IdentityString + '"]').val(elesConfig[i].OptionValues);
                                }

                                if (elesConfig[i].Value != null && elesConfig[i].Value != "") {
                                    $('#orderform_' + formId + ' [name="' + elesConfig[i].IdentityString + '"]').val(elesConfig[i].Value.toString());
                                }
                            }
                        } catch (e) {
                    ...
                    case 1:
                        ...
                        if ($('#orderform_' + formId + ' [name="' + elesConfig[i].IdentityString + '"]').length > 0) {
                            ...
                            if ($('#orderform_' + formId + ' [name="' + elesConfig[i].IdentityString + '"]')[0].type == "radio") {
                                ...
                                $('input:radio[name="' + elesConfig[i].IdentityString + '"][value="' + elesConfig[i].Value + '"]').attr('checked', true)
                            } else if ($('#orderform_' + formId + ' [name="' + elesConfig[i].IdentityString + '"]')[0].type == "checkbox") {
                                if (elesConfig[i].Value != null) {
                                    var values = elesConfig[i].Value.split(',');
                                    for (var z = 0; z < values.length; z++) {
                                        $('input:checkbox[name="' + elesConfig[i].IdentityString + '"][value="' + values[z] + '"]').attr('checked', true)
                                    }
                                }
        ...
        money_cn: function (num) {
            if ($.isNumeric(num)) {
                ...
                return strOutput.replace(/零角零分$/, '整').replace(/零[仟佰拾]/g, '零').replace(/零{2,}/g, '零').replace(/零([亿|万])/g, '$1').replace(/零+元/, '元').replace(/亿零{0,3}万/, '亿').replace(/^元/, "零元").replace(/零分/, "");
            }
            return '';
        },
...
$(function () {
    ...
    $(document).ajaxError(function (event, request, settings) {
        if (request.status != 200 && settings.url != "" && showAjaxErrorAlert) {
            $.messager.alert("通讯出错", "在与页面:\"" + settings.url + "\"通讯时出现【" + request.status + "】错误，请刷新页面", "error", function () {
                $.ajaxEnd();
            });
        }
    });
    ...
    $(document).ajaxSuccess(function (event, request, settings) {
        if (showAjaxErrorAlert) {
            ...
            if (request.getAllResponseHeaders().indexOf('X-Responded-JSON: {"status":401') > 0) {
                $.messager.alert("授权失败", "在与页面:\"" + settings.url + "\"通讯时出现权限错误，您没有访问该路径的权限，可能登陆状态已过期，请刷新页面", "error", function () {
                    $.ajaxEnd();
        ...
        $('#password_dialog_change form').ajaxSubmit({
            ...
            success: function (data) {
                if (data.ResultCode == 1) {
                    ...
                    $.messager.alert('错误',"用户名或密码有误", 'warning');
                }
            }
        })
    }
